...
{
    public partial class OtherProd : Form
    {
        public OtherProd()
        {
            InitializeComponent();
        ...
        List<int> OthersList = new List<int>();

        private void OtherProd_Load(object sender, EventArgs e)
        {
            Variable.TempCartThing.Clear();
            OthersList.Add(311001);
            OthersList.Add(311008);
            for (int i = 311021; i < 311025; i++) { OthersList.Add(i); }
            int x = 0;
            foreach (ComboBox c in this.Controls.OfType<ComboBox>().OrderBy(c => c.TabIndex))
            {
                ...
                c.Tag = OthersList[x];                             //將對應商品的ID丟進對應的ComboBox中
